...
@Component({
  selector: 'ngx-list-rol-encargado-evento',
  templateUrl: './list-rol_encargado_evento.component.html',
  styleUrls: ['./list-rol_encargado_evento.component.scss'],
})
export class ListRolEncargadoEventoComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.eventoService.get('rol_encargado_evento/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.eventoService.delete('rol_encargado_evento', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
