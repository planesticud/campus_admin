...
export class ViewExperienciaLaboralComponent implements OnInit {
  ...
  constructor(
    ...
    private documentoExp: DocumentoService,
    private nuxeoExp: NuxeoService,
    ...
    this.experienciaService.get('soporte_experiencia_laboral/?query=ExperienciaLaboral.Persona:' + this.ente +
      ...
      .subscribe(res => {
        if (res !== null) {
          ...
          this.nuxeoExp.getDocumentoById$(soportes, this.documentoExp)
            .subscribe(responseExp => {
              this.documentosExpSoporte = <Array<any>>responseExp;

              if (Object.values(this.documentosExpSoporte).length === data.length) {
                              ...
                              this.ubicacionesService.get('lugar/' + organizacion_info.Ubicacion.UbicacionEnte.Lugar)
                                .subscribe(pais => {
                                  if (pais !== null && JSON.stringify(pais).toString() !== '[{}]') {
                                    ...
                                    element.Documento = this.cleanURL(this.documentosExpSoporte['DocumentoExp' + contador] + '');
                                    contador++;
                                    this.info_experiencia_laboral = data2;
                                  }
                                },
