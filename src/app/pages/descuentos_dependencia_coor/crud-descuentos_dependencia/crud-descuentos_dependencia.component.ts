...
import { FORM_DESCUENTOS_DEPENDENCIA_COOR } from './form-descuentos_dependencia';
...
@Component({
  selector: 'ngx-crud-descuentos-coordinacion',
...
export class CrudDescuentosDependenciaComponent implements OnInit {
  ...
  constructor(private translate: TranslateService,
    ...
    this.formDescuentosDependencia = FORM_DESCUENTOS_DEPENDENCIA_COOR;
    this.construirForm();
  ...
  loadOptionsPeriodoId(): void {
    ...
    this.core.get('periodo?query=Activo:true&limit=0')
      .subscribe(res => {
        if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
                    ...
                    this.core.get('periodo/' + this.element.PeriodoId).subscribe(res4 => {
                      if (res4 !== null && JSON.stringify(res4).toString() !== '[{}]') {
                        ...
                        if (this.element.PeriodoId.Activo.toString() !== 'true') {
                          for (let i = 0; i < this.formDescuentosDependencia.campos.length; i++) {
                            this.formDescuentosDependencia.campos[i].deshabilitar = true;
                          }
                          this.formDescuentosDependencia.campos[this.getIndexForm('PeriodoId')].opciones.push(<Periodo>res4);
                        }
                      }
  ...
  validarForm(event) {
    if (event.valid) {
      if (this.info_descuentos_dependencia === undefined) {
        this.createDescuentosDependencia(event.data.DescuentosDependenciaCoor);
      } else {
        this.updateDescuentosDependencia(event.data.DescuentosDependenciaCoor);
      }
    }
  }
