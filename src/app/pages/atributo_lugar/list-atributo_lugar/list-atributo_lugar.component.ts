...
@Component({
  selector: 'ngx-list-atributo-lugar',
  templateUrl: './list-atributo_lugar.component.html',
  styleUrls: ['./list-atributo_lugar.component.scss'],
  })
export class ListAtributoLugarComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.ubicacionService.get('atributo_lugar/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.ubicacionService.delete('atributo_lugar', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
