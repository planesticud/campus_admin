...
export class ListDependenciaComponent implements OnInit {
  ...
  cargarCampos() {
    this.settings = {
      ...
      columns: {
        ...
        TelefonoDependencia: {
          title: this.translate.instant('GLOBAL.telefono_dependencia'),
          width: '15%',
          valuePrepareFunction: (value) => {
            return value;
          },
  ...
  loadData(): void {
    this.programaOikosService.get('dependencia/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.programaOikosService.delete('dependencia', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
