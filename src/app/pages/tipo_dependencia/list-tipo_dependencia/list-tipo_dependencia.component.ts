...
@Component({
  selector: 'ngx-list-tipo-dependencia',
  templateUrl: './list-tipo_dependencia.component.html',
  styleUrls: ['./list-tipo_dependencia.component.scss'],
})
export class ListTipoDependenciaComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.programaOikosService.get('tipo_dependencia/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.programaOikosService.delete('tipo_dependencia', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
