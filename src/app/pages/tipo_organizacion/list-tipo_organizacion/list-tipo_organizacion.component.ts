...
@Component({
  selector: 'ngx-list-tipo-organizacion',
  templateUrl: './list-tipo_organizacion.component.html',
  styleUrls: ['./list-tipo_organizacion.component.scss'],
  })
export class ListTipoOrganizacionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.organizacionService.get('tipo_organizacion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.organizacionService.delete('tipo_organizacion', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
