...
@Component({
  selector: 'ngx-list-nivel-formacion',
  templateUrl: './list-nivel_formacion.component.html',
  styleUrls: ['./list-nivel_formacion.component.scss'],
})
export class ListNivelFormacionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.programaAcademicoService.get('nivel_formacion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.programaAcademicoService.delete('nivel_formacion', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
