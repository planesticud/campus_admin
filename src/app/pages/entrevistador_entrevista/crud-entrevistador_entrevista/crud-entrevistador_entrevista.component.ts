...
export class CrudEntrevistadorEntrevistaComponent implements OnInit {
  ...
  construirForm() {
    ...
    for (let i = 0; i < this.formEntrevistadorEntrevista.campos.length; i++) {
      ...
      this.formEntrevistadorEntrevista.campos[i].placeholder = this.translate.instant('GLOBAL.placeholder_'
        + this.formEntrevistadorEntrevista.campos[i].label_i18n);
    }
  }
  ...
  loadOptionsEntrevistador(): void {
    // const entrevistador: Array<any> = [];

      this.entrevistaService.get('entrevistador/?limit=0')
        .subscribe(res => {
              if (Object.keys(res[0]).length > 0) {
                const entrevistadores = <Array<any>>res;
                entrevistadores.forEach( (entrevistador: any) => {
                  this.docenteService.get('docentes_por_proyecto/' + entrevistador.ProgramaAcademicoId)
                    .subscribe(res_docente => {
                      ...
                      if (Object.keys(listDocentes).length !== 0) {
                        const arrayFilter = listDocentes.filter(e => e.identificacion + '' === entrevistador.PersonaId + '');
                        const persona = <any>arrayFilter[0];
                        entrevistador['Nombre'] = persona.nombres + ' ' + persona.apellidos;
                      }
                    });
            });
  ...
  public loadEntrevistadorEntrevista(): void {
    if (this.entrevistador_entrevista_id !== undefined && this.entrevistador_entrevista_id !== 0) {
      this.entrevistaService.get('entrevistador_entrevista/?query=id:' + this.entrevistador_entrevista_id)
        .subscribe(res => {
          if (res !== null) {
            this.docenteService.get('docentes_por_proyecto/' + res[0].EntrevistadorId.ProgramaAcademicoId)
              .subscribe(res_docente => {
                const listDocentes = <Array<any>>res_docente['docentesCollection']['docentes'];
                if (Object.keys(listDocentes).length !== 0) {
                  // const arrayFilter = listDocentes.filter(e => e.identificacion + '' === res[0].EntrevistadorId.PersonaId + '');
                  // const persona = <Persona>arrayFilter[0];

                  res[0].inscripcion_id = res[0].EntrevistaId.InscripcionId;
                  this.info_entrevistador_entrevista = res[0];
                }
            });
          }
        });
