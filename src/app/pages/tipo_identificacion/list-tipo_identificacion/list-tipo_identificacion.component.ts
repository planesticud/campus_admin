...
@Component({
  ...
  templateUrl: './list-tipo_identificacion.component.html',
  styleUrls: ['./list-tipo_identificacion.component.scss'],
})
export class ListTipoIdentificacionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.enteService.get('tipo_identificacion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.enteService.delete('tipo_identificacion', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'TipoIdentificacion deleted');
