...
export class ListTipoProyectoComponent implements OnInit {
  ...
  constructor(private translate: TranslateService, private inscripcion: InscripcionService, private toasterService: ToasterService) {
    this.loadData();
    this.cargarCampos();
  ...
  loadData(): void {
    this.inscripcion.get('tipo_proyecto/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.inscripcion.delete('tipo_proyecto', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
