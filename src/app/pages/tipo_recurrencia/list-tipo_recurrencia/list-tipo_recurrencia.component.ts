...
@Component({
  selector: 'ngx-list-tipo-recurrencia',
  templateUrl: './list-tipo_recurrencia.component.html',
  styleUrls: ['./list-tipo_recurrencia.component.scss'],
})
export class ListTipoRecurrenciaComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.eventoService.get('tipo_recurrencia/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.eventoService.delete('tipo_recurrencia', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
