...
import { FORM_DOCUMENTO_PROGRAMA_COOR } from './form-documento_programa';
...
@Component({
  selector: 'ngx-crud-documento-coordinacion',
...
export class CrudDocumentoProgramaComponent implements OnInit {
  ...
  constructor(private translate: TranslateService,
    ...
    this.formDocumentoPrograma = FORM_DOCUMENTO_PROGRAMA_COOR;
    this.construirForm();
                    ...
                    this.core.get('periodo/' + this.element.PeriodoId).subscribe(res4 => {
                      if (res4 !== null && JSON.stringify(res4).toString() !== '[{}]') {
                        ...
                        if (this.element.PeriodoId.Activo.toString() !== 'true') {
                          for (let i = 0; i < this.formDocumentoPrograma.campos.length; i++) {
                            this.formDocumentoPrograma.campos[i].deshabilitar = true;
                          }
                          this.formDocumentoPrograma.campos[this.getIndexForm('PeriodoId')].opciones.push(<Periodo>res4);
                        }
                      }
  ...
  validarForm(event) {
    if (event.valid) {
      if (this.info_documento_programa === undefined) {
        this.createDocumentoPrograma(event.data.DocumentoProgramaCoor);
      } else {
        this.updateDocumentoPrograma(event.data.DocumentoProgramaCoor);
      }
    }
  }
