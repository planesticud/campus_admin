...
@Component({
  selector: 'ngx-list-tipo-discapacidad',
  templateUrl: './list-tipo_discapacidad.component.html',
  styleUrls: ['./list-tipo_discapacidad.component.scss'],
})
export class ListTipoDiscapacidadComponent implements OnInit {
  ...
  loadData(): void {
    this.personaService.get('tipo_discapacidad/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.personaService.delete('tipo_discapacidad', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
