...
@Component({
  selector: 'ngx-list-tipo-recibo',
  templateUrl: './list-tipo_recibo.component.html',
  styleUrls: ['./list-tipo_recibo.component.scss'],
  })
export class ListTipoReciboComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.reciboService.get('tipo_recibo/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.reciboService.delete('tipo_recibo', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
