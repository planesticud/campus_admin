...
@Component({
  selector: 'ngx-list-concepto-academico',
  templateUrl: './list-concepto_academico.component.html',
  styleUrls: ['./list-concepto_academico.component.scss'],
})
export class ListConceptoAcademicoComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.coreService.get('concepto_academico/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.coreService.delete('concepto_academico', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
