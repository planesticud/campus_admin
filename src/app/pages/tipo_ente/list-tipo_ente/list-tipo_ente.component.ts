...
@Component({
  selector: 'ngx-list-tipo-ente',
  templateUrl: './list-tipo_ente.component.html',
  styleUrls: ['./list-tipo_ente.component.scss'],
})
export class ListTipoEnteComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.enteService.get('tipo_ente/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.enteService.delete('tipo_ente', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
            this.showToast('info', 'deleted', 'TipoEnte deleted');
