...
@Component({
  selector: 'ngx-list-tipo-dedicacion',
  templateUrl: './list-tipo_dedicacion.component.html',
  styleUrls: ['./list-tipo_dedicacion.component.scss'],
})
export class ListTipoDedicacionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.experienciaService.get('tipo_dedicacion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.experienciaService.delete('tipo_dedicacion', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
