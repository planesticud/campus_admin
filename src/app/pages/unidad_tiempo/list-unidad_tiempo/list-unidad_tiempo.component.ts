...
@Component({
  selector: 'ngx-list-unidad-tiempo',
  templateUrl: './list-unidad_tiempo.component.html',
  styleUrls: ['./list-unidad_tiempo.component.scss'],
})
export class ListUnidadTiempoComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.coreService.get('unidad_tiempo/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.coreService.delete('unidad_tiempo', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
