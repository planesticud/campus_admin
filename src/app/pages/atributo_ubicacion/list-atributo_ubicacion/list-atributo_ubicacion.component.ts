...
@Component({
  selector: 'ngx-list-atributo-ubicacion',
  templateUrl: './list-atributo_ubicacion.component.html',
  styleUrls: ['./list-atributo_ubicacion.component.scss'],
  })
export class ListAtributoUbicacionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.enteService.get('atributo_ubicacion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.enteService.delete('atributo_ubicacion', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
