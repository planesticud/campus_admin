...
@Component({
  selector: 'ngx-list-estado-recibo',
  templateUrl: './list-estado_recibo.component.html',
  styleUrls: ['./list-estado_recibo.component.scss'],
  })
export class ListEstadoReciboComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.reciboService.get('estado_recibo/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.reciboService.delete('estado_recibo', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
