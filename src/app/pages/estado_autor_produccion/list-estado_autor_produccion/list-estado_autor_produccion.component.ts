...
@Component({
  selector: 'ngx-list-estado-autor-produccion',
  templateUrl: './list-estado_autor_produccion.component.html',
  styleUrls: ['./list-estado_autor_produccion.component.scss'],
  })
export class ListEstadoAutorProduccionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.produccionService.get('estado_autor_produccion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.produccionService.delete('estado_autor_produccion', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
