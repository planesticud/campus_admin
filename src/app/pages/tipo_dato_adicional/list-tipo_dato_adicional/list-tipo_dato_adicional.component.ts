...
@Component({
  selector: 'ngx-list-tipo-dato-adicional',
  templateUrl: './list-tipo_dato_adicional.component.html',
  styleUrls: ['./list-tipo_dato_adicional.component.scss'],
})
export class ListTipoDatoAdicionalComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.coreService.get('tipo_dato_adicional/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.coreService.delete('tipo_dato_adicional', event.data).subscribe(res => {
          if (res !== null && JSON.stringify(res).toString() !== '[{}]') {
            this.loadData();
