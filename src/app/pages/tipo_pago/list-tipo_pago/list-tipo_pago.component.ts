...
@Component({
  selector: 'ngx-list-tipo-pago',
  templateUrl: './list-tipo_pago.component.html',
  styleUrls: ['./list-tipo_pago.component.scss'],
  })
export class ListTipoPagoComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.reciboService.get('tipo_pago/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.reciboService.delete('tipo_pago', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
