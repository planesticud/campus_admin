...
@Component({
  selector: 'ngx-list-tipo-produccion',
  templateUrl: './list-tipo_produccion.component.html',
  styleUrls: ['./list-tipo_produccion.component.scss'],
  })
export class ListTipoProduccionComponent implements OnInit {
  uid: number;
  ...
  loadData(): void {
    this.produccionService.get('tipo_produccion/?limit=0').subscribe(res => {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      if (willDelete.value) {
        this.produccionService.delete('tipo_produccion', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
